...
YAHOO_TICKERS = ["SPY", "HYG", "FXY", "GLD", "^MOVE", "^VIX", "^VIX3M", "^VIX9D", "^TNX"]
...
def fetch_yahoo(ticker: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for d, row in df.iterrows():
            v = float(row[price_col].iloc[0]) if hasattr(row[price_col], 'iloc') else float(row[price_col])
            if math.isnan(v):
                continue
...
def compute_derived(all_series: dict[str, list[dict[str, Any]]]) -> dict[str, list[dict[str, Any]]]:
    ...
    dgs10_for_gold = to_dict(all_series.get("DGS10", []))
    derived["GOLD_10Y_RATIO"] = [
        {"date": d, "value": gold[d] / dgs10_for_gold[d]}
        for d in sorted(gold.keys() & dgs10_for_gold.keys())
        if dgs10_for_gold[d] > 0
    ]
    ...
    dgs10 = dgs10_for_gold  # reuse, already loaded above
...
def main() -> int:
    ...
    # Crypto — CoinGecko free tier is rate-limited (429); fallback to previous series.json
    _prev_series = {}
    _prev_path = OUT_DIR / "series.json"
    if _prev_path.exists():
        try:
            with _prev_path.open() as f:
                _prev_series = json.load(f)
        except Exception:
            pass

    print("[INFO] Stablecoin aggregate (CoinGecko)")
    stablecoin = fetch_stablecoin_aggregate()
    if stablecoin:
        all_series["STABLECOIN_MCAP_B"] = stablecoin
    elif "STABLECOIN_MCAP_B" in _prev_series and _prev_series["STABLECOIN_MCAP_B"]:
        all_series["STABLECOIN_MCAP_B"] = _prev_series["STABLECOIN_MCAP_B"]
        print("[WARN]   → CoinGecko rate-limited, using previous series.json fallback "
              f"({len(all_series['STABLECOIN_MCAP_B'])} rows)")
    else:
        all_series["STABLECOIN_MCAP_B"] = []

    print("[INFO] BTC price (CoinGecko)")
    btc = fetch_btc_price()
    if btc:
        all_series["BTC_USD"] = btc
    elif "BTC_USD" in _prev_series and _prev_series["BTC_USD"]:
        all_series["BTC_USD"] = _prev_series["BTC_USD"]
        print("[WARN]   → CoinGecko rate-limited, using previous series.json fallback "
              f"({len(all_series['BTC_USD'])} rows)")
    else:
        all_series["BTC_USD"] = []
    ...
    # ── Manifest integrity check (graded: core → hard fail, aux → banner) ──
    # Core series feed traffic lights, triggers, regime, or position waterfall.
    # Missing/empty core → abort (return 1).  Auxiliary series → banner warning only.
    #
    # MOVE tiering (v3.5.1): ^MOVE+MOVE→AUX (Yahoo rate-limit → banner + CASC fallback visible),
    # MOVE_PROXY→CORE (FRED DGS2/DGS10 input, dual failure = hard abort).
    _FRED_CORE = {e["id"] for e in FRED_SERIES}
    _COMPUTED_CORE = {"SOFR_IORB", "EFFR_IORB", "HY_IG_RATIO", "MOVE_PROXY"}
    _OTHER_CORE   = {"STABLECOIN_MCAP_B", "GOLDAMGBD228NLBM"}
    CORE_SERIES = _FRED_CORE | (set(YAHOO_TICKERS) - {"^MOVE"}) | _COMPUTED_CORE | _OTHER_CORE

    AUX_SERIES  = {"^MOVE", "MOVE", "GOLD_10Y_RATIO", "BTC_USD"}

    # ── Purge stale aux-degraded sidecar from previous run (prevents false banner) ──
    banner_path = OUT_DIR / "_aux_degraded.txt"
    banner_path.unlink(missing_ok=True)

    actual_keys = set(all_series.keys())

    # ── Core: hard fail ──
    core_missing = sorted(CORE_SERIES - actual_keys)
    core_empty   = sorted(k for k in actual_keys & CORE_SERIES if not all_series.get(k))
    if core_missing or core_empty:
        if core_missing:
            print(f"[FATAL] CORE series MISSING: {core_missing}", file=sys.stderr)
        if core_empty:
            print(f"[FATAL] CORE series EMPTY (0 obs): {core_empty}", file=sys.stderr)
        print(f"[FATAL] Core manifest incomplete — aborting. Fix fetch before proceeding.",
              file=sys.stderr)
        return 1
    print(f"[OK] Core manifest: {len(actual_keys & CORE_SERIES)}/{len(CORE_SERIES)} series present & non-empty")

    # ── Aux: banner warning (top of report, not buried in footer) ──
    aux_missing = sorted(AUX_SERIES - actual_keys)
    aux_empty   = sorted(k for k in actual_keys & AUX_SERIES if not all_series.get(k))
    aux_issues: list[str] = []
    if aux_missing:
        aux_issues.append(f"missing: {aux_missing}")
    if aux_empty:
        aux_issues.append(f"empty: {aux_empty}")
    if aux_issues:
        msg = "[WARN] AUXILIARY series degraded — report will still be generated | " + "; ".join(aux_issues)
        print(msg, file=sys.stderr)
        # Write banner file so daily_report.py can pick it up as a top-of-report banner
        banner_path.write_text(msg + "\n")
        print(f"[INFO] Aux degradation banner written to {banner_path}")
