...
Usage:
    python scripts/fetch_yahoo_local.py          # pull & save
    python scripts/fetch_yahoo_local.py --push   # pull, save, commit & push
"""
...
def fetch_ticker(ticker: str) -> list[dict]:
    ...
    try:
        ...
        for d, row in df.iterrows():
            rows.append({"date": str(d.date()), "value": float(row["Close"].iloc[0])})
