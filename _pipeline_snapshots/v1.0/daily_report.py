...
SERIES_FREQ: dict[str, str] = {
    ...
    "WALCL": "weekly",
    "WTREGEN": "weekly",
    "DTWEXBGS": "daily",  # FRED D-prefix = daily; H.10 weekly is a different series
...
# Yahoo series IDs
_YAHOO_VINTAGE_IDS = [SPY_ID, HYG_ID, FXY_ID, "GLD", "^MOVE"]
...
def data_source_vintage_str(vint: dict) -> str:
    """Format vintage line, e.g. 'FRED: 05-29 (T-3) | Yahoo: 06-01 (T-1)'."""
    parts = []
    if vint["fred_date"]:
        parts.append(f"FRED: {vint['fred_date'][-5:]} (T-{vint['fred_tn']})")
    if vint["yahoo_date"]:
        parts.append(f"Yahoo: {vint['yahoo_date'][-5:]} (T-{vint['yahoo_tn']})")
    return " | ".join(parts)
...
def compute_position(abcd: dict, v35: dict, *, casc: dict | None = None) -> dict:
    ...
    if vts_gate == "候选":
        ...
        s_vts_note += f" · 正常化={vts_norm_days}d, 曾倒挂={vts_pos.get('was_inverted', False)}"
...
def format_paper_trade_md(
    ...
    if casc:
        ...
        if vts_pt and vts_pt.get("ratio_vix_vix3m") is not None:
            lines.append(f"- VTS §0.8: 期限结构={vts_pt.get('structure','N/A')} ({vts_pt['ratio_vix_vix3m']:.3f}) · 再入场={vts_pt.get('re_entry_gate','未启用')}")
        # RCV summary
        rcv_pt = casc.get("rcv", {})
...
def format_markdown_report(
    ...
    if casc:
        ...
        lines.append("")
        lines.append("| 腿 | 标的 | 当前值 | 5dΔ | 形态 | 突变? | 状态 |")
        lines.append("|----|------|--------|------|------|-------|------|")
        for leg_name in ["VIX", "MOVE", "HY OAS 20dΔ", "FX"]:
            leg = casc["legs"].get(leg_name, {})
            if leg_name == "VIX":
        ...
        if vts:
            ...
            lines.append(f"> [VTS §0.8: 期限结构={vts_struct}({vts_ratio_str}) · 前端={vts_front}({f'{vts_fr_ratio:.3f}' if vts_fr_ratio is not None else 'N/A'}) · 再入场门={vts_gate}]")
