...
def build_html(data: dict, run_date: str) -> str:
    ...
    trig_a = hy_bp >= 300 or ig_bp >= 85
    trig_a_text = f"当前：{'已触发' if trig_a else '未触发'} (HY {hy_oas} / IG {ig_oas})"
    trig_a_cls = "badge-red" if trig_a else "badge-green"

    trig_b_text = "当前：部分压力，但未到系统性阈值"
    trig_b_cls = "badge-orange"

    fxy_pct = _to_num(fxy_5d) or 0
    trig_c = fxy_pct > 2.5 or casc >= 1
    trig_c_text = f"当前：{'已触发' if trig_c else '未触发'} (D端 {d_light} / CASC {casc}/4)"
    trig_c_cls = "badge-red" if trig_c else "badge-green"
...
<div class="container">
    ...
    <div class="section border-orange-thick">
        ...
        <div class="grid-2">
            ...
            <div class="box theme-red">
                ...
                <div class="box-content"><ul>
                    <li>VIX9D / VIX = {vix9d_display} → 前端急性</li>
    ...
    <div class="section border-yellow-thick">
        ...
        <div class="grid-3">
            <div class="box-3">
                <div class="box-3-header">触发器 A：信用开始恶化</div>
                <ul>
                    <li>HY OAS 脱离自满区: &gt; 300bp</li>
                    <li>IG OAS 脱离自满区: &gt; 85bp</li>
                    <li>若继续走阔 → B 端转坏</li>
                </ul>
                <div class="status-badge {trig_a_cls}">{trig_a_text}</div>
            </div>
            <div class="box-3">
                <div class="box-3-header">触发器 B：流动性继续收紧</div>
                <ul>
                    <li>EFFR-IORB 继续上行至 🔴</li>
                    <li>SOFR-IORB 接近或升破 0bp</li>
                    <li>A 端从偏紧走向真正压力</li>
                </ul>
                <div class="status-badge {trig_b_cls}">{trig_b_text}</div>
            </div>
            <div class="box-3">
                <div class="box-3-header">触发器 C：跨资产 / 跨境扩散</div>
                <ul>
                    <li>D 端启动 (如 FXY 5d &gt; +2.5%)</li>
                    <li>CASC 从 0/4 升至 1/4 以上</li>
                    <li>风险从利率扩散到外汇与多资产</li>
                </ul>
                <div class="status-badge {trig_c_cls}">{trig_c_text}</div>
            </div>
        </div>
