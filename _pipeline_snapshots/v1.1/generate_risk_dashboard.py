...
# Markdown Dashboard
# ═══════════════════════════════════════════════════════════════════════════
def format_dashboard_md(data_date, run_date, abcd, pos, v35, casc, vts, rcv, lock, rate_path):
    ...
    # Systemic stage
    if lock_state == "agree-systemic": stage, stage_l, stage_c = "systemic", "⚠️ 已进入系统性 — §0.7 Role B", "🔴"
    elif vts.get("structure","") in ("倒挂","倒挂·急性"): stage, stage_l, stage_c = "pre-systemic","🟡 前端积聚·系统性前兆","🟠"
    elif lock_state == "agree-front": stage, stage_l, stage_c = "front","近端事件·非系统性","🟡"
    else: stage, stage_l, stage_c = "calm","低风险·双端平静","🟢"

    # Verdict
    verdicts = {
        "systemic": "系统已进入系统性风险阶段。VTS倒挂+RCV长端/全曲线→Role B确认触发。激进降风险。",
        "pre-systemic": "前端压力积聚，未进系统性。等RCV翻long-led或acute-broad叠VTS倒挂→升档。",
        "front": "非系统性·近端事件。双探针前端一致→等CPI/Fed落地看前端是fade还是扩散。要盯的翻转点：RCV→long-led/acute-broad叠VTS倒挂→agree-systemic。",
        "calm": "低风险·双端平静。维持benchmark仓位。",
    }
    ...
    lines.append("---\n## ① 近端事件风险\n")
    evt = []; nxt = ""
    if vts.get("front_structure") in ("前端急性","前端紧张"): evt.append(f"VIX9D/VIX={vix9d_r:.3f} **{vts['front_structure']}**")
    elif vix9d_r: evt.append(f"VIX9D/VIX={vix9d_r:.3f} 前端平静")
    ...
    lines.append(f"| 利率路径 | DGS2−IORB={rp_gap}bp {rp_label}{' 5dΔ'+str(rp_5d)+'bp' if rp_5d else ''} |\n")
    ...
    lines.append(f"|--------|------|---------|")
    lines.append(f"| **A 信用** | HY/IG OAS走阔脱离自满 | {t_a} {'已触发' if t_a in ('🟠','🔴') else '未触发'} (HY/IG ⚠️自满) |")
    t_b_txt = f"{t_b} {'部分压力' if t_b=='🟠' else '已触发' if t_b=='🔴' else '未触发'}"
    lines.append(f"| **B 流动性** | EFFR-IORB 🟠/🔴+DUR5≥5 | {t_b_txt} (EFFR-IORB={effr_v}bp DUR5={dur5_e}/5) |")
    t_c_txt = f"{t_c} {'已触发' if t_c=='🔴' else '需关注' if t_c=='🟠' else '未触发'}"
    lines.append(f"| **C 跨资产/跨境** | CASC≥2+VTS+RCV互锁 | {t_c_txt} (CASC{casc_conf}/4·VTS={vts.get('structure','N/A')}·互锁={lock_state}) |\n")
    ...
    lines.append(f"| RCV | {rcv.get('character','N/A')} · sev={rcv.get('severity','N/A')} · tilt={rcv.get('tilt','N/A')} |")
    lines.append(f"| 互锁 | {lock_state} — {lock.get('state_label','N/A')} |")
    lines.append(f"| C端 | {casc_label} |")
    lines.append(f"\n> **最终判断**：{verdicts.get(stage,'')}\n")

    lines.append(f"---\n*ABCD v3.5 风险演化看板 | {run_date} | FRED+Yahoo*")
    return "\n".join(lines)
...
def generate_png(data_date, run_date, abcd, pos, casc, vts, rcv, lock, rate_path, out_path):
    ...
    else: stage_s, stage_t = "#2E8B57","CALM · Low Risk"
    ...
    txt(ax1,0.3,7.0,f"Event: VIX9D/VIX={vix9d:.3f} * {front_s}  |  VIX={vix_val:.1f}(5dD{vix_leg.get('delta_5d',0):+.1f})  |  MOVE={move_val:.0f}  |  Rate: DGS2-IORB={rp_gap}bp",9.5,tc_mid)
    ...
    metrics = [
        ...
        (f"RCV: {rcv.get('character','N/A')}", f"sev={rcv.get('severity','N/A')} tilt={rcv.get('tilt','N/A')}"),
    ...
    stg_key = "systemic" if lock_state=="agree-systemic" else ("pre-systemic" if vts.get("structure","") in ("倒挂","倒挂·急性") else ("front" if lock_state=="agree-front" else "calm"))
    box(ax4,0.3,1.5,9.4,1.8,stage_s,stage_s,0.1)
    txt(ax4,0.6,2.8,f"VERDICT: {verdicts.get(stg_key,'')}",10,tc_dark,True)

    # Footer
    txt(ax4,0.3,0.3,f"ABCD v3.5 Risk Evolution Dashboard · {run_date} · FRED + Yahoo",8,tc_dim)
...
def main():
    ...
    # 1) Markdown
    md = format_dashboard_md(data_date, run_date, abcd, pos, v35, casc, vts, rcv, lock, rate_path)
